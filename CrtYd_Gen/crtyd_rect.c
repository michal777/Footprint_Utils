...
int main (void)
{
	...
	for (j = 0; strcmp (footprint_list[j].d_name, "||\\//||") != 0; ++j)
	{
		...
		for (fgets (buffer, BUFFER_SIZE, file_in); !feof (file_in); fgets (buffer, BUFFER_SIZE, file_in))
		{
			...
			if (line_start_x - CRTYD_CLEARANCE < left)
				left = line_start_x - CRTYD_CLEARANCE;
			if (line_end_x - CRTYD_CLEARANCE < left)
				left = line_end_x - CRTYD_CLEARANCE;
			if ((pad_pos_x - pad_size_x / 2 - CRTYD_CLEARANCE) < left)
				left = (pad_pos_x - pad_size_x / 2 - CRTYD_CLEARANCE);
			if (line_start_x - CRTYD_CLEARANCE > right)
				right = line_start_x + CRTYD_CLEARANCE;
			if (line_end_x + CRTYD_CLEARANCE > right)
				right = line_end_x + CRTYD_CLEARANCE;
			if ((pad_pos_x + pad_size_x / 2 + CRTYD_CLEARANCE) > right)
				right = (pad_pos_x + pad_size_x / 2 + CRTYD_CLEARANCE);
			if (line_start_y - CRTYD_CLEARANCE < top)
				top = line_start_y - CRTYD_CLEARANCE;
			if (line_end_y - CRTYD_CLEARANCE < top)
				top = line_end_y - CRTYD_CLEARANCE;
			if ((pad_pos_y - pad_size_y / 2 - CRTYD_CLEARANCE) < top)
				top = (pad_pos_y - pad_size_y / 2 - CRTYD_CLEARANCE);
			if (line_start_y - CRTYD_CLEARANCE > bottom)
				bottom = line_start_y + CRTYD_CLEARANCE;
			if (line_end_y - CRTYD_CLEARANCE > bottom)
				bottom = line_end_y + CRTYD_CLEARANCE;
		...
		for (fgets (buffer, BUFFER_SIZE, file_in); !feof (file_in); fgets (buffer, BUFFER_SIZE, file_in))
		{
			if ((strncmp("  (fp_line ", buffer, strlen("  (fp_line ")) == 0) && ok == 0)
			{
				ok = 1;
				fprintf (file_out, "  (fp_line (start %lg %lg) (end %lg %lg) (layer F.CrtYd) (width 0.05))\n", n2d(r05(left)), n2d(r05(top)), n2d(r05(left)), n2d(r05(bottom)));
				fprintf (file_out, "  (fp_line (start %lg %lg) (end %lg %lg) (layer F.CrtYd) (width 0.05))\n", n2d(r05(right)), n2d(r05(top)), n2d(r05(right)), n2d(r05(bottom)));
				fprintf (file_out, "  (fp_line (start %lg %lg) (end %lg %lg) (layer F.CrtYd) (width 0.05))\n", n2d(r05(left)), n2d(r05(top)), n2d(r05(right)), n2d(r05(top)));
				fprintf (file_out, "  (fp_line (start %lg %lg) (end %lg %lg) (layer F.CrtYd) (width 0.05))\n", n2d(r05(left)), n2d(r05(bottom)), n2d(r05(right)), n2d(r05(bottom)));
			}
