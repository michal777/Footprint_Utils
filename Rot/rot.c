	...
	for (j = 0; strcmp (footprint_list[j].d_name, "||\\//||") != 0; ++j)
	{
		...
		for (fgets (buffer, BUFFER_SIZE, file_in); !feof (file_in); fgets (buffer, BUFFER_SIZE, file_in))
		{
			...
			//			printf("lw %d\n", line_width);
				...
				fprintf (file_out, "  (fp_line (start %lg %lg) (end %lg %lg) (layer F.SilkS) (width %lg))\n", n2d(0 - line_start_y), n2d(line_start_x), n2d(0 - line_end_y), n2d(line_end_x), n2d(line_width));
			}
			else if ((strncmp("  (pad ", buffer, strlen("  (pad ")) == 0))
			{
				...
				fprintf (file_out, "%sat %lg %lg) (size %lg %lg%s", buff_beg, n2d(0 - pad_pos_y), n2d(pad_pos_x), n2d(pad_size_y), n2d(pad_size_x), buff_end);
			}
			else if ((strncmp("    (rotate (xyz ", buffer, strlen("    (rotate (xyz ")) == 0))
			{
				for (i = 0; buffer[i] != '\n'; ++i)
				{
					if (buffer[i + 1] == '\n')
					{
						for (k = 0; buffer[i - k] != ' '; ++k)
							{;}
						sscanf(buffer + i - k, "%lg", & angle_double);
						angle = (int) (angle_double * 1000000);
					}
				}
				fprintf (file_out, "    (rotate (xyz 0 0 %lg))\n", n2d(angle + 90 * 1000000));
			}
			else
